...
function DropdownMenu.New(Config, Dropdown, Element, Type)
	...
	function DropdownModule:Add(Tab)
		if Config.Window.Destroyed then
			return
		end

		table.insert(Dropdown.Values, Tab)
		local Index = #Dropdown.Tabs + 1
	...
	function DropdownModule:Remove(ValueToRemove)
		...
		for Index, TabMain in ipairs(Dropdown.Tabs) do
			if TabMain.Original == ValueToRemove or TabMain.Name == ValueToRemove then
				if TabMain.UIElements and TabMain.UIElements.TabItem then
					TabMain.UIElements.TabItem:Destroy()
				end
				FoundIndex = Index
				break
			end
		...
		if FoundIndex then
			table.remove(Dropdown.Tabs, FoundIndex)
